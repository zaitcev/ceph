...
{
//  PaxosServiceMessage *req = static_cast<PaxosServiceMessage*>(op->get_req());
  int mon = get_leader();
...
void Monitor::dispatch_op(MonOpRequestRef op)
{
  ...
  if (!op->get_req_session()->is_capable("mon", MON_CAP_R)) {
  ...
  switch (op->get_req()->get_type()) {
    ...
    case MSG_MON_PAXOS:
      {
        ...
        if (!op->is_src_mon() ||
            !op->get_req_session()->is_capable("mon", MON_CAP_X)) {
          //can't send these!
          pm->put();
          break;
        }
    ...
    case MSG_MON_ELECTION:
      ...
      if (op->get_req_session() &&
          !op->get_req_session()->is_capable("mon", MON_CAP_X)) {
        dout(0) << "MMonElection received from entity without enough caps!"
          << op->get_req_session()->caps << dendl;
        //m->put();
        break;
      }
      if (!is_probing() && !is_synchronizing()) {
